...
contract SubscriptionToken {
  ...
  event Subscribed(address indexed _subscriber);
  event SubscriptionPaid(address indexed _subscriber);
  event Unsubscribed(address indexed _subscriber);
  event SubscriptionCanceled(address indexed _subscriber);
  ...
  function pay(address _subscriber) public {
    ...
    if (balanceOf[_subscriber] < subscriptionPrice) {
      ...
      emit SubscriptionCanceled(_subscriber);
      return;
    }
    ...
    scheduler.scheduleCall(address(this), 0, 50000, 100, 10, abi.encodeWithSignature("pay(address)", _subscriber));
